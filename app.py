...
if "upload_paths" not in st.session_state:
    st.session_state.upload_paths = []  # lista de dicts: {name, path}
...
def read_e1_counts(xlsx_path: str) -> Tuple[int | None, int | None]:
    ...
    Tenta várias células por causa de merges/estilos.
    """
    try:
...
# Função que processa UM PDF e devolve o resultado para UI + summary
def process_one_pdf(pdf_path: str, final_dir: Path) -> Dict:
    """
    Executa process_pdf(pdf_path) e devolve:
    {
      'pdf_name': str,
      'created': [paths],
      'req_count': int,
      'samples_total': int,
      'discrepancies': [ {'xlsx':name,'proc':int,'exp':int} ]
    }
    """
...
if st.session_state.stage == "idle":
    ...
    if start and uploads:
        # Guarda PDFs para disco (para não perder buffers em reruns)
    ...
    # Processamento paralelo por PDF
    start_ts = time.time()
    results: List[Dict] = []
    futures = {}
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as ex:
        for idx, it in enumerate(items):
            # Cada worker define OUTPUT_DIR próprio (a tua pipeline usa esta env)
        ...
        for fut in as_completed(futures):
            ...
            try:
                ...
                res = {
                    "pdf_name": os.path.basename(items[idx]["path"]),
                    "created": [],
                    "req_count": 0,
                    "samples_total": 0,
                    "discrepancies": [],
    ...
    # Agrega para ZIP e summary
    all_excel = []
    summary_lines = []

    # Ordena resultados pela ordem de upload original
    results_sorted = [None]*len(items)
    for res in results:
        # coloca pela posição original
        try:
    ...
    for res in results:
        ...
        # linha principal do PDF
        if res["discrepancies"]:
            warning_count += 1
            summary_lines.append(f"{pdf_name}: {req_count} requisições, {samples_total} amostras ⚠️ {len(res['discrepancies'])} discrepância(s).")
        else:
            summary_lines.append(f"{pdf_name}: {req_count} requisições, {samples_total} amostras")

        # sublinhas ↳ por .xlsx (com ⚠️ quando aplicável)
        produced = []
        for p in res["created"]:
            all_excel.append(p)
            produced.append(Path(p).name)

        # mapa rápido {xlsx: (proc,exp)} para marcar as com discrepância
        disc_map = {d["xlsx"]: (d["proc"], d["exp"]) for d in res["discrepancies"]}

        # primeiro as com discrepância (na ordem), depois as normais
    ...
    # ZIP + download
    # (debug_files: procuramos nas pastas temporárias criadas pelos workers)
