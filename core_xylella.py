...
# Diretório de saída temporário (definido pelo app via env)
        ...
        for r in result:
            if isinstance(r, str):
                ...
                entries.append({
                    ...
                    "expected": r[2] if len(r) > 2 else None,
                    "discrepancy": bool(r[3]) if len(r) > 3 else False
                })
...
def process_pdf_with_stats(pdf_path: str):
    ...
    for i, e in enumerate(entries):
        stats["per_req"].append({
            ...
            "diff": (e.get("processed", 0) - (e.get("expected") or 0)) if e.get("expected") is not None else None,
...
def build_zip_with_summary(excel_files, debug_files, summary_text):
    """Gera ZIP com Excel + debug + summary.txt."""
    import io
    import zipfile
